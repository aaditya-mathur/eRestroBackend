...
function validate_promo_code($promo_code, $user_id, $final_total, $wallet_balance_used, $partner_id = NULL)
{
    if (isset($promo_code) && !empty($promo_code)) {
        ...
        $promo_code = $t->db->select(
            'pc.*, 
    COUNT(o.id) as promo_used_counter,
    (SELECT COUNT(user_id) 
        FROM orders 
        WHERE user_id = ' . $user_id . ' 
        AND promo_code = "' . $promo_code . '"
    ) AS user_promo_usage_counter'
        )
            ->from('promo_codes pc')
            ->join('orders o', 'o.promo_code = pc.promo_code', 'left')

            ->group_start()   // Start grouped WHERE (promo conditions)
            ->where('pc.promo_code', $promo_code)
            ->where('pc.status', '1')
            ->where('pc.start_date <=', date('Y-m-d'))
            ->where('pc.end_date >=', date('Y-m-d'))
            ->group_end()     // End grouped WHERE

            // Partner optional (works only for that promo)
            ->group_start()
            ->where('pc.partner_id', $partner_id)
            ->or_where('pc.partner_id', NULL)
            ->or_where('pc.partner_id', '')
            ->or_where('pc.partner_id', '0')
            ->group_end()
...
function validate_order_status($order_ids, $status, $table = 'order_items', $user_id = null)
{
    ...
    if (in_array(strtolower(trim($status)), $check_status)) {
        ...
        if ($canceling_delivered_item == true) {
            ...
            $response['error'] = true;
            $response['message'] = "You cannot cancel delivered order item.";
            $response['data'] = array();
            return $response;
        }
...
function get_cart_total($user_id, $product_variant_id = false, $is_saved_for_later = '0', $address_id = '', $city_id = '')
{
    $t = &get_instance();
    ...
    for ($i = 0; $i < count($data); $i++) {
        ...
        $add_ons = get_cart_add_ons($data[$i]['id'], $data[$i]['product_id'], $user_id, $city_id);
        if (!empty($add_ons)) {
            $sum = 0;
...
function get_cart_add_ons($variant_id, $product_id, $user_id, $city_id = NULL)
{
    if (isset($city_id) && !empty($city_id)) {

        $data = fetch_details(['ca.user_id' => $user_id, "ca.product_id" => $product_id, "ca.product_variant_id" => $variant_id, 'ca.city_id' => $city_id], "cart_add_ons ca", "*", null, null, null, null, null, null, "product_add_ons pa", "pa.id=ca.add_on_id", false, "ca.add_on_id");
    } else {
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $rider_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $download_thermal_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $partner_id = null, $is_pending = false, $rider_city_id = NULL)
{
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        if (!empty($order_item_data)) {
            ...
            if (!empty($order_items_snapshot[0]['order_items_snapshot'])) {
                $order_details[$i]['order_items'] = json_decode(stripslashes($order_items_snapshot[0]['order_items_snapshot']), true);
            } else {
...
function get_delivery_charge($address_id, $user_id, $partner_id)
{
    $t = &get_instance();
    $charge = "0";
    if (is_exist(['id' => $address_id], 'addresses')) {
        ...
        $points = fetch_details(['id' => $partner_id], 'users', 'latitude,longitude');
        $system_settings = get_settings('system_settings', true);
...
function calculate_distance($latitudeFrom, $longitudeFrom, $latitudeTo, $longitudeTo)
{
    ...
    $long1 = deg2rad(floatval($longitudeFrom));
    $long2 = deg2rad(floatval($longitudeTo));
    $lat1 = deg2rad(floatval($latitudeFrom));
    $lat2 = deg2rad(floatval($latitudeTo));
...
function fetch_partners($filter = [], $user_id = null, $limit = NULL, $offset = '', $sort = 'u.id', $order = 'DESC', $search = NULL)
{
    ...
    if (!empty($rows)) {
        // sort rows based on is_restro_open (1 comes first, 0 later)
        usort($rows, function ($a, $b) {
            return $b['is_restro_open'] <=> $a['is_restro_open'];
        });
    }
