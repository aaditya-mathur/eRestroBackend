    ...
    public function add_promo_code()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_partner()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($_POST['no_of_users']) && !empty($_POST['no_of_users']) && $_POST['no_of_users'] <= 0) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'No. of users must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
                }
                if (isset($_POST['minimum_order_amount']) && !empty($_POST['minimum_order_amount']) && $_POST['minimum_order_amount'] <= 0) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Minimum Order Amount must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
                }
                if (isset($_POST['max_discount_amount']) && !empty($_POST['max_discount_amount']) && $_POST['max_discount_amount'] <= 0) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Maximum Discount Amount must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
                }
                if (isset($_POST['no_of_repeat_usage']) && !empty($_POST['no_of_repeat_usage']) && $_POST['no_of_repeat_usage'] <= 0) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'No. of Repeat Usage must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
                }
