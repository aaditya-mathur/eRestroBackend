            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($_POST['commission']) && !empty($_POST['commission'])) {
                    if ($_POST['commission'] < 0) {
                        $response["error"]   = true;
                        $response["message"] = "Fixed Commission per Order must be greater than zero";
                    ...
                    if (isset($_POST['percentage']) && !empty($_POST['percentage'])) {
                        ...
                        if ($_POST['max_commission'] < 0) {
                            $response["error"]   = true;
                            $response["message"] = "Max Commission must be greater than zero";
                            $response['csrfName'] = $this->security->get_csrf_token_name();
                            $response['csrfHash'] = $this->security->get_csrf_hash();
                            $response["data"] = array();
                            echo json_encode($response);
