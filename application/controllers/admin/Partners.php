...
class Partners extends CI_Controller
{
    ...
    public function delete_partner()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!empty($partner_data)) {
                file_exists($partner_data[0]['profile']) && is_file($partner_data[0]['profile']) ?  unlink(FCPATH . $partner_data[0]['profile']) : "";
                file_exists($partner_data[0]['national_identity_card']) && is_file($partner_data[0]['national_identity_card']) ?  unlink(FCPATH . $partner_data[0]['national_identity_card']) : "";
                file_exists($partner_data[0]['address_proof']) && is_file($partner_data[0]['address_proof']) ?  unlink(FCPATH . $partner_data[0]['address_proof']) : "";
            }
    ...
    public function add_partner()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('global_commission', 'Global Commission', 'trim|required|xss_clean|numeric');
            $this->form_validation->set_rules('cooking_time', 'Cooking Time', 'trim|required|xss_clean|numeric');
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($_POST['cooking_time']) && !empty($_POST['cooking_time']) && $_POST['cooking_time'] <= 0) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Cooking time must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
                }
                if (isset($_POST['mobile']) && !empty($_POST['mobile']) && $_POST['mobile'] <= 0) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Mobile number must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
                }
                if (isset($_POST['account_number']) && !empty($_POST['account_number']) && $_POST['account_number'] <= 0) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Account numbermust be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
                }
