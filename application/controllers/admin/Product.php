...
class Product extends CI_Controller
{
    ...
    public function add_product()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if(isset($_POST['available_time']) && $_POST['available_time'] == 'on'){
                if(strtotime($_POST['product_start_time']) >= strtotime($_POST['product_end_time'])){
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'End time must be greater than start time';
                    print_r(json_encode($this->response));
                    return false;
                }
            }
            if(isset($_POST['minimum_order_quantity']) && !empty($_POST['minimum_order_quantity']) && $_POST['minimum_order_quantity'] <= 0){
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Minimum order quantity must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
            }
            if(isset($_POST['calories']) && !empty($_POST['calories']) && $_POST['calories'] <= 0){
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Calories must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
            }
            if(isset($_POST['total_allowed_quantity']) && !empty($_POST['total_allowed_quantity']) && $_POST['total_allowed_quantity'] <= 0){
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Total allowed quantity must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
            }
            if(isset($_POST['simple_price']) && !empty($_POST['simple_price']) && $_POST['simple_price'] <= 0){
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Simple price must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
            }
            if(isset($_POST['simple_special_price']) && !empty($_POST['simple_special_price']) && $_POST['simple_special_price'] <= 0){
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Simple special price must be greater than zero';
                    print_r(json_encode($this->response));
                    return false;
            }
                    ...
                    while (($row = fgetcsv($handle, 10000, ",")) != FALSE) //get row values
                    {
                        if ($temp != 0) {
                            ...
                            if (empty($row[15])) {
                                if($row[15] < 0){
                                    $this->response['error'] = true;
                                    $this->response['message'] = 'Partner ID cant be negative at row ' . $temp;
                                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                                    print_r(json_encode($this->response));
                                    return false;
                            ...
                            if (isset($row[16]) && $row[16] == '') {
                                if($row[16] < 0){
                                    $this->response['error'] = true;
                                    $this->response['message'] = 'Tag id cant be negative at row ' . $temp;
                                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                                    print_r(json_encode($this->response));
                                    return false;
