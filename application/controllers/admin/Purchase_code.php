...
class Purchase_code extends CI_Controller
{
    ...
    public function validator()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            // ✅ Validation: At least one must be entered
            if (empty($purchase_code_app) && empty($purchase_code_web)) {
                $this->response['error'] = true;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $this->response['message'] = "Please enter at least one purchase code (App or Web).";
                echo json_encode($this->response);
                return; // stop execution
            }

            // ✅ Handle App purchase code validation
            if (!empty($purchase_code_app)) {
                $url = "https://validator.wrteam.in/home/validator_new?purchase_code=$purchase_code_app&domain_url=" . base_url() . "&item_id=" . ASHTANGA;
                $result = curl($url);
                ...
                if (!empty($result['body'])) {
                    if (isset($result['body']['error']) && $result['body']['error'] == 0) {
                        ...
                        if (empty($doctor_brown)) {
                            $doctor_brown = [
                                'code_bravo' => $result["body"]["purchase_code"],
                                'time_check' => $result["body"]["token"],
                                'code_adam' => $result["body"]["username"],
                                'dr_firestone' => $result["body"]["item_id"]
                            ];
                            insert_details(['variable' => 'doctor_brown', 'value' => json_encode($doctor_brown)], 'settings');
                        }
                        $this->response = [
                            'error' => false,
                    ...
                    $this->response = [
                        ...
                        'csrfName' => $this->security->get_csrf_token_name(),
                        'csrfHash' => $this->security->get_csrf_hash(),
                        'message' => "Something went wrong. Please contact the Super Admin."
                    ];
                    echo json_encode($this->response);
                    return;
                }
            ...
            // ✅ Handle Web purchase code validation
            if (!empty($purchase_code_web)) {
                $url = "https://validator.wrteam.in/home/validator_new?purchase_code=$purchase_code_web&domain_url=" . base_url() . "&item_id=" . ASHTANGAWEB;
                $result = curl($url);
                ...
                if (!empty($result['body'])) {
                    if (isset($result['body']['error']) && $result['body']['error'] == 0) {
                        ...
                        if (empty($doctor_brown_web)) {
                            ...
                            insert_details(['variable' => 'doctor_brown_web', 'value' => json_encode($doctor_brown_web)], 'settings');
                        }
                        $this->response = [
                            'error' => false,
                            'csrfName' => $this->security->get_csrf_token_name(),
