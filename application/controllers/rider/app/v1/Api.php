    ...
    */
    ...
    public function get_pending_orders()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            // Fetch user's serviceable cities
            $city_id = fetch_details(['id' => $user_id], "users", 'serviceable_city');
            $cities = !empty($city_id[0]['serviceable_city']) ? explode(',', $city_id[0]['serviceable_city']) : [];
            ...
            // ✅ Loop through each city and fetch orders individually
            foreach ($cities as $city) {
                ...
                $order_details = fetch_orders(
                    ...
                    null,
                    true,
                    $city // ✅ Send one city at a time
                );

                if (!empty($order_details) && !empty($order_details['order_data'])) {
                    $merged_data = array_merge($merged_data, $order_details['order_data']);
                    $total_orders += (int)$order_details['total'];
                }
            }
            ...
            if (!empty($merged_data)) {
                $this->response['error'] = false;
                $this->response['message'] = 'Data retrieved successfully';
                $this->response['total'] = $total_orders;
                $this->response['data'] = $merged_data;
            } else {
                $this->response['error'] = true;
                $this->response['message'] = 'Order Does Not Exists';
                $this->response['total'] = "0";
