...
class Product_model extends CI_Model
{
    ...
    public function get_product_details($flag = NULL, $partner_id = NULL, $p_status = NULL)
    {
        ...
        ->join("product_variants", "product_variants.product_id = p.id", "left");
        ...
        foreach ($pro_search_res as $row) {
            ...
            if ($row['status'] == '1') {
                ...
                $operate .= '<a class="btn btn-warning btn-xs update_active_status mr-1 mb-1 ml-1" data-table="products" title="Deactivate" href="javascript:void(0)" data-id="' . $row['pid'] . '" data-status="' . $row['status'] . '" ><i class="fa fa-toggle-on"></i></a>';
            } else  if ($row['status'] == '0') {
                $tempRow['status'] = '<a class="badge badge-danger text-white" >Inactive</a>';
                $operate .= '<a class="btn btn-secondary mr-1 mb-1 ml-1 btn-xs update_active_status" data-table="products" href="javascript:void(0)" title="Active" data-id="' . $row['pid'] . '" data-status="' . $row['status'] . '" ><i class="fa fa-toggle-off"></i></a>';
            }
    ...
    public function get_product_add_ons($product_id = "", $from_api = false)
    {
        ...
        foreach ($cat_search_res as $row) {
            $row = output_escaping($row);
                $operate = "";
            if ($row['status'] == '1') {
                $operate .= '<a class="btn btn-success btn-xs update_active_status mr-1 mb-1" data-table="product_add_ons" title="Deactivate" href="javascript:void(0)" data-id="' . $row['id'] . '" data-status="' . $row['status'] . '" ><i class="fa fa-toggle-on"></i></a>';
            } else {
                $operate .= '<a class="btn btn-secondary mr-1 mb-1 btn-xs update_active_status" data-table="product_add_ons" href="javascript:void(0)" title="Active" data-id="' . $row['id'] . '" data-status="' . $row['status'] . '" ><i class="fa fa-toggle-off"></i></a>';
            }
