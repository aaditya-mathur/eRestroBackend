...
class Transaction_model extends CI_Model
{
    ...
    function get_transactions_list($user_id = '', $group_id = 2)
    {
        ...
        if (isset($_GET['search']) && $_GET['search'] != '') {
            ...
            if (isset($_GET['user_type']) && $_GET['user_type'] == 'partner') {
                $multipleWhere['rd.partner_name'] = $search;
            }
        }
        ...
        if (isset($_GET['user_id']) && !empty($_GET['user_id'])) {
            $where = array_merge($where, ['users.id' => $_GET['user_id']]);
        }
        ...
        if (isset($group_id[0]['id']) && !empty($group_id[0]['id'])) {
            $where = array_merge($where, ['ug.group_id' => $group_id[0]['id']]);
        } else {
            $where = array_merge($where, ['ug.group_id' => $group_id]);
        }
        ...
        if (isset($_GET['user_type']) && $_GET['user_type'] == "partner") {
            $count_res->join('partner_data rd', "rd.user_id = users.id", "left");
        }
        ...
        if (isset($_GET['user_type']) && $_GET['user_type'] == "partner") {
            $search_res->join('partner_data rd', "rd.user_id = users.id", "left");
        }
        ...
        $txn_search_res = $search_res->order_by($sort, $order)
            ->limit($limit, $offset)
            ->get('transactions')
            ->result_array();
