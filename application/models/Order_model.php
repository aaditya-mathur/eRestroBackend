                    ...
                    if ($current_status == 'delivered') {
                        ...
                        if (!empty($order)) {
                            ...
                            if ($rider_id > 0) {
                                ...
                                update_details(['rider_payment' => $commission], ['order_id' => $row['id']], 'order_transaction');
                                /* give tip to rider if set in order */
                                if ($order[0]['delivery_tip'] > 0) {
                                    update_details(['delivery_tip' => $order[0]['delivery_tip']], ['order_id' => $row['id']], 'order_transaction');
    ...
    public function place_order($data)
    {
        ...
        if (!empty($product_variant)) {
            ...
            if (isset($data['promo_code']) && !empty($data['promo_code'])) {
                ...
                if($promocode_type[0]['promocode_type']=='partner'){
                    $promo_code = validate_promo_code($data['promo_code'], $data['user_id'], $data['total'], $data['wallet_balance_used'],$product_variant[0]['partner_id']);
    ...
    public function get_orders_list(
        ...
        if (isset($is_pending)) {
            if ($is_pending == true && !empty($rider_city_id)) {
                ...
                $rider_city_ids = explode(',', $rider_city_id); // convert "1,58,2" to array [1,58,2]
                $count_res->where_in('po.city_id', $rider_city_ids);
                $count_res->where_in('o.active_status', ['confirmed', 'preparing']);
                // Exclude orders where this rider has already rejected
                if (!empty($rider_id)) {
                    $count_res->where("(FIND_IN_SET(" . (int)$rider_id . ", po.rejected_riders) = 0 OR po.rejected_riders IS NULL)", null, false);
                }
            }
        }
        ...
        if (isset($is_pending)) {
            if ($is_pending == true && !empty($rider_city_id)) {
                ...
                // Exclude orders where this rider has already rejected
                if (!empty($rider_id)) {
                    $search_res->where("(FIND_IN_SET(" . (int)$rider_id . ", po.rejected_riders) = 0 OR po.rejected_riders IS NULL)", null, false);
                }
            }
        }
        ...
        foreach ($user_details as $row) {
            if (!empty($row['items'])) {
                ...
                $status_html = '';
                if (!empty($row['items'][0]['order_status'])) {
                    $decoded_status = json_decode($row['items'][0]['order_status'], true);
                    if (is_array($decoded_status)) {
                        foreach ($decoded_status as $st) {
                            $status_name = isset($st[0]) ? ucfirst(str_replace('_', ' ', $st[0])) : '';
                            $status_time = isset($st[1]) ? date('d-m-Y h:i A', strtotime($st[1])) : '';
                            $status_html .= "<strong>{$status_name}</strong> : {$status_time}<br>------------------<br>";
                        }
                    } else {
                        $status_html = "No status found";
                    }
                } else {
                    $status_html = "No status available";
                }

                // Replace the old $status variable usage with this
                $tempRow['status'] = $status_html;
                ...
                $tempRow['delivery_tip'] = (isset($row['delivery_tip']) && !empty($row['delivery_tip'])) ? $currency_symbol . ' ' . $row['delivery_tip'] : $currency_symbol . ' ' . "0";
