...
class Ticket_model extends CI_Model
{
    ...
    function get_ticket_list()
    {
        ...
        // ✅ FIX 1: count tickets by t.id (not u.id)
        $count_res = $this->db->select('COUNT(DISTINCT t.id) as `total`')
            ->join('ticket_types tty', 'tty.id=t.ticket_type_id', 'left')
            ->join('users u', 'u.id=t.user_id', 'left');

        // ✅ FIX 2: use or_like instead of or_where for consistent filtering
        ...
        // -----------------------------
        // MAIN SEARCH QUERY (same)
        // -----------------------------
        $search_res = $this->db->select('t.*,DATE(t.date_created) as date_created,tty.title,u.username')
            ->join('ticket_types tty', 'tty.id=t.ticket_type_id', 'left')
            ->join('users u', 'u.id=t.user_id', 'left');
    ...
    function get_ticket_type_list()
    {
        ...
        if (isset($multipleWhere) && !empty($multipleWhere)) {
            $count_res->group_start();
            $count_res->or_like($multipleWhere); // ✅ changed from or_where() → or_like()
            $count_res->group_end();
        }
        if (isset($where) && !empty($where)) {
            $count_res->where($where);
