...
class Attribute_model extends CI_Model
{
    ...
    //     ];
    //     if (isset($data['edit_attribute_id'])) {
    //         // Get the current attribute_value IDs in DB
    //         $this->db->select('id');
    //         $this->db->where('attribute_id', $data['edit_attribute_id']);
    //         $existing_ids = array_column($this->db->get('attribute_values')->result_array(), 'id');

    //         // IDs from form
    //         $posted_ids = isset($data['value_id']) ? array_filter($data['value_id']) : [];

    //         // Find which IDs were removed in the form
    //         $deleted_ids = array_diff($existing_ids, $posted_ids);

    //         // Delete those IDs
    //         if (!empty($deleted_ids)) {
    //             $this->db->where_in('id', $deleted_ids)->delete('attribute_values');
    //         }

    //         // Continue with update/insert logic
