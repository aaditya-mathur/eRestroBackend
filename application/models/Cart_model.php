...
{
    public function add_to_cart($data, $check_status = TRUE)
    {
    ...
    public function remove_from_cart($data)
    {
        ...
        if (isset($data['user_id']) && !empty($data['user_id'])) {
            ...
            //  Add city_id condition if available
            ...
            if (isset($data['product_variant_id'])) {
                $product_variant_id = explode(',', $data['product_variant_id']);
                // if (isset($data['city_id']) && !empty($data['city_id'])) {
                //     delete_details(['user_id' => $data['user_id'], 'city_id' => $data['city_id']], "cart_add_ons", "product_variant_id", $product_variant_id);
                // } else {
            ...
            if (isset($data['user_id']) && !empty($data['user_id']) && !isset($data['product_variant_id'])) {
                // if (isset($data['city_id']) && !empty($data['city_id'])) {
                //     delete_details(['user_id' => $data['user_id'], 'city_id' => $data['city_id']], "cart_add_ons");
                // } else {
    ...
    public function get_user_cart($user_id, $is_saved_for_later = 0, $product_variant_id = '', $city_id = '')
    {
        ...
        $q = $this->db->join('product_variants pv', 'pv.id=c.product_variant_id')
            ...
            ->where(['c.user_id' => $user_id,'c.city_id' => $city_id, 'p.status' => '1', 'pv.status' => 1, 'pd.status' => 1, 'qty !=' => '0', 'is_saved_for_later' => $is_saved_for_later]);
    ...
    public function re_order_cart($data, $check_status = TRUE)
    {
        $data = escape_array($data);
