...
class Partner_model extends CI_Model
{
    ...
    function get_partners_list()
    {
        ...
        if (isset($_GET['search']) and $_GET['search'] != '') {
            $search = $_GET['search'];
            $multipleWhere = ['u.`id`' => $search, 'u.`username`' => $search, 'pd.`partner_name`' => $search, 'u.`email`' => $search, 'u.`mobile`' => $search, 'pd.`address`' => $search, 'u.`balance`' => $search];
        }
        ...
        foreach ($offer_search_res as $row) {
            ...
            if ($row['type'] == 1)
                $tempRow['type'] = "<label class='badge badge-success'>Veg</label>";
            else if ($row['type'] == 2)
                $tempRow['type'] = "<label class='badge badge-danger'>Non-Veg</label>";
            else if ($row['type'] == 3)
                $tempRow['type'] = "<label class='badge badge-warning'>Veg & Non-Veg</label>";
            else if ($row['type'] == 0)
                $tempRow['type'] = "<label class='badge badge-primary'>None</label>";
            ...
            $licenceProofimagePaths = json_decode($row['licence_proof'], true);
            if(isset($licenceProofimagePaths) && !empty($licenceProofimagePaths)){
                foreach ($licenceProofimagePaths as $licenceProofimage) {
                    $licenceProoffullImagePath = base_url() . $licenceProofimage;
                    $tempRow['licence_proof'] =  '<img class="partner_profile_image" src="' . $licenceProoffullImagePath . '" alt="Image">';
                }
            }
    ...
    function settle_partner_commission()
    {
        ...
        */
        ...
        $data = $this->db->select("o.id as order_id,date(o.date_added) as order_date,oi.product_variant_id,oi.partner_id,o.final_total,o.total,o.delivery_charge,o.tax_amount,o.promo_code,o.promocode_type,o.promo_discount,o.wallet_balance")
        ...
        foreach ($data as $row) {
            $rider_commission = fetch_details(['order_id' => $row['order_id']], 'order_transaction', 'rider_payment,delivery_tip');
            $rider_payment = isset($rider_commission[0]['rider_payment']) && !empty($rider_commission[0]['rider_payment']) ? $rider_commission[0]['rider_payment'] : 0;
            $delivery_tip = isset($rider_commission[0]['delivery_tip']) && !empty($rider_commission[0]['delivery_tip']) ? $rider_commission[0]['delivery_tip'] : 0;
            $wallet_balance = isset($row['wallet_balance']) && !empty($row['wallet_balance']) ? $row['wallet_balance'] : 0;
            $total_amount = $row['total'] + $row['tax_amount']  - $rider_payment;
            ...
            if(isset($row['promocode_type']) && !empty($row['promocode_type']) && $row['promocode_type'] == 'partner'){
                ...
                $commission_amt = $commission_amt - $promocode_discount;

            }
            // print_r($commission_amt);
            // die;
            ...
            if ($response['error'] == false) {
                update_details(['admin_payment' => $commission_amt,'partner_payment'=> $transfer_amt, 'settelment_status' => 1], ['order_id' => $row['order_id']], 'order_transaction');
    ...
    function settle_admin_commission()
    {
        ...
        foreach ($data as $row) {
            ...
            $commission_amt = floatval($total_amount) * ($commission_pr / 100);

            // $transfer_amt = $row['final_total'] - $commission_amt;
            $transfer_amt = floatval($total_amount) - $commission_amt;

            if(isset($row['promocode_type']) && !empty($row['promocode_type']) && $row['promocode_type'] == 'partner'){
                $promocode_discount = $row['promo_discount'];
                $transfer_amt = $total_amount - $commission_amt - $promocode_discount;
            }else{
                $promocode_discount = $row['promo_discount'];
                $transfer_amt = $total_amount - $commission_amt;
                $commission_amt = $commission_amt - $promocode_discount;
    ...
    public function top_partners()
    {
        ...
        foreach ($total_sale as &$sale) {
            ...
            $sale['total_sales'] = $currency . ' ' . number_format(
                $sale['total_sales'],
                2
