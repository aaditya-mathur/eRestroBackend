  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(result, function (a, b) {
        ...
        html +=
          '<div class="form-group row"><div class="col col-xs-12"><label class="control-label">Price :</label><input type="number" name="variant_price[]" oninput="validateNumberInput(this)" class="col form-control price varaint-must-fill-field" min="0" step="0.01"></div><div class="col col-xs-12"><label class="control-label">Special Price :</label><input type="number" name="variant_special_price[]" class="col form-control discounted_price" min="0" step="0.01"></div>';
  ...
  if (data[0].variant_ids) {
    ...
    $.each(data, function (a, b) {
      ...
      if (
        ...
        html +=
          '<div class="form-group row"><div class="col col-xs-12"><label class="control-label">Price :</label><input type="number" name="variant_price[]" oninput="validateNumberInput(this)" class="col form-control price varaint-must-fill-field" value="' +
          b.price +
          '" min="0" step="0.01"></div><div class="col col-xs-12"><label class="control-label">Special Price :</label><input type="number" name="variant_special_price[]" oninput="validateNumberInput(this)" class="col form-control discounted_price" min="0" value="' +
          b.special_price +
...
$(document).on("click", ".view_ticket", function (e, row) {
  ...
  $("#user_name").html(
    username && username.trim() !== "" && !username.toLowerCase().startsWith("data-date_created")
      ? username
      : "(N/A)"
  );
  ...
  Swal.fire({
    ...
    preConfirm: function () {
      return new Promise((resolve, reject) => {
        $.ajax({
          type: "GET",
          url: base_url + from + "/attributes/delete_attribute_combination",
          data: { id: cat_id },
          dataType: "json"
        })
          .done(function (response, textStatus) {
...
// Add new attribute value input block
$(document).on("click", "#add_attribute_val", function (e) {
  e.preventDefault();
  show_attribute_values();
});

// Display the attribute value fields
function show_attribute_values(attribute_values = [], attribute_value_ids = []) {
  var html = "";

  if (attribute_values.length > 0) {
    $.each(attribute_values, function (key, val) {
      html += `<div class="form-group row attribute-row">
        <div class="col-sm-10">
          <input type="text" class="form-control" placeholder="Values Name" name="value_name[]" value="${val}">
          <input type="hidden" name="value_id[]" value="${attribute_value_ids[key]}">
        </div>
        <div class="col-sm-2 text-right">
          <button type="button" class="btn btn-tool remove-attribute-value" data-id="${attribute_value_ids[key]}">
    ...
    html = `<div class="form-group row attribute-row">
      ...
      <div class="col-sm-2 text-right">
        <button type="button" class="btn btn-tool remove-attribute-value">
          <i class="text-danger far fa-times-circle fa-2x"></i>
        </button>
      </div>
    </div>`;
...
$(document).on("click", ".remove-attribute-value", function (e) {
  ...
  const inputVal = $row.find('input[name="value_name[]"]').val().trim();

  // If field has a value, confirm before removing
  if (inputVal !== "") {
    const confirmDelete = confirm("This value will be removed. Continue?");
    if (!confirmDelete) {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result.error == false) {
        iziToast.success({
          message:
            '<span class="messages_style">' + result.message + "</span> "
        });
      } else {
        // Split the multiple <p> messages into an array
        var tempDiv = $("<div>").html(result.message);
...
$("#sign_up_restro_form").on("submit", function (e) {
  ...
  if (
    confirm(
      "Are you sure you want to register to the system? Check the details you have filled"
    )
  ) {
    $.ajax({
      type: "POST",
      ...
      success: function (result) {
        ...
        if (result.error == false) {
          ...
          // Split all <p> tags and show each message separately
          var messages = $(result.message).filter("p");
          if (messages.length > 0) {
            ...
            // Fallback: if there are no <p> tags, show the full message
            iziToast.error({
              message: "<span>" + result.message + "</span>",
              position: "topRight",
              timeout: 4000
...
function closeTab() {
  // Try multiple methods to close/navigate away

  // Method 1: Try to close if window was opened by script
  if (window.opener) {
    window.close();
    return;
  }

  // Method 2: Try standard close (works in some browsers)
  try {
    window.close();
  } catch (e) {
  ...
  // Method 4: Navigate to a common page or show message
  // You can customize this URL to your app's home page
  window.location.href = 'about:blank';

  // Alternative: Show a message if nothing works
  setTimeout(() => {
    alert('Please close this tab manually or navigate to your desired page.');
  }, 100);
}
...
$(document).ready(function () {
  $('button[type="reset"]').on('click', function (e) {
    e.preventDefault(); // Prevent default browser reset
    ...
    $('#slider_type').val(' ').trigger('change'); // Set to "Select Type"
    $('#offer_type').val(' ').trigger('change'); // Set to "Select Type"

    // Reset category dropdown to "Select category"
    $('#category_parent').val(' ').trigger('change');
    $("#start_date, #end_date").val("");
    $("#title").val("");

    // Reset product search dropdown (if using Select2 or similar)
    $('.search_product').val(null).trigger('change');
    $('.search_offer_product').val(null).trigger('change');

    // Clear uploaded image preview
    $('.image-upload-section .image img').attr('src', '');
  });
});

$(document).ready(function () {
  // When the modal is shown, attach reset handler
  $('#ticket_type_modal').on('click', 'button[type="reset"]', function (e) {
    e.preventDefault();
    const form = $(this).closest('form');
  ...
  // Optional: clear modal entirely on close, so next open starts fresh
  $('#ticket_type_modal').on('hidden.bs.modal', function () {
    $(this).find('.modal-body').html('');
  });
});


// reset featured section form while updating
$(document).on('click', '.modal .btn[type="reset"]', function (e) {
  e.preventDefault(); // prevent default reset behavior
  var $form = $(this).closest('form');

  // Clear all text, number, email, hidden, password inputs
  $form.find('input[type="text"], input[type="number"], input[type="email"], input[type="hidden"], input[type="password"]').val('');

  // Clear textarea
  $form.find('textarea').val('');
  $("#start_date, #end_date").val("");
  // Reset select dropdowns (Bootstrap or normal)
  $form.find('select').prop('selectedIndex', 0).trigger('change');

  // Clear multiple selects or Select2 if used
  $form.find('select[multiple]').val([]).trigger('change');

  // Clear image upload preview sections if any
  $form.find('.image-upload-section').html('');

  // Optionally reset validation or error messages
...
$('form.form-submit-event').on('reset', function () {
  setTimeout(() => {
    const $valField = $('#attribute_values, #attribute_value');

    // Clear manually
    $valField.val('').trigger('change');

    // Remove tags if rendered in a span/div (common pattern)
    $valField.siblings('.bootstrap-tagsinput, .tagify, .selectize-input').find('.tag, .tagify__tag, .item').remove();
  }, 100);
});
